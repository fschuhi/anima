...
- ~~**Safely replace the active PDF after a hot open request.** Implemented in `AppDelegate.loadDocument(url:)`: Finder, Dock drop, and "Open With" requests replace the displayed PDF while Anima is running, using only the first URL if several arrive. Replacement is skipped while an AppKit modal is active. The outgoing document's PDF-text search, comment search, annotation/card emphasis, text selection, and Delete-key target are cleared; bookmarks are reloaded for the incoming file. If parsing fails, the current document is preserved and an alert is shown.~~
  *Completed: basic replacement works. X-Ray mode and persistent highlight mode are reset during replacement so the new document starts in the default reading state.*
