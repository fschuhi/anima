...
- ~~**Centralize far-jump execution before implementing JumpStack.** Done 2026-07-19 as `TARGET_ARCHITECTURE.md` Phase C step 9. `AnimaPDFView` now owns the seam: three `farJump(to:)` overloads mirroring PDFKit's own `go(to:)` vocabulary (`PDFPage`, `PDFSelection`, `PDFDestination`), each calling a private `recordPreJumpPosition()` before delegating. Target discovery stayed with its existing local owners; only the final transition moved. **Five** call sites were rerouted, not six: each `F3` shares its find's method, so the six sources collapse to `showGotoPageDialog`, `showFindResult`, and `jumpToBookmark` in `AnimaPDFView`, `navigateToCommentSearchResult` in `MainViewController`, and the same-document `pdf://` path in `AppDelegate`, whose Phase B marker comment is now gone. `restore(toPageIndex:)` and §6.3 step 5 stay outside the seam by design; the dead `reloadDocument()` keeps its own direct `go(to:)` pending its separate deletion item. **Contract established:** every far jump executes through the seam, and step 10 records the pre-jump position inside `recordPreJumpPosition()` -- an empty stub for now, which is exactly what made step 9 behavior-neutral by construction rather than by testing. **Decided against:** flattening all three targets into a single `farJump(to: PDFDestination)`. It is the tidier end-state, but it would have changed four of five primitives, swapped PDFKit's reveal-the-selection scrolling for find/`F3` to top-of-viewport destination scrolling, and added top-of-page geometry at three sites -- all inside a step chartered to change nothing visible. Revisit only if step 10 needs a target richer than a page index, or if a fourth target type appears; note that a future return to JumpStack persistence would argue for page granularity rather than against it. No `NavigationManager`: the seam has one owner and no independent responsibility.~~
